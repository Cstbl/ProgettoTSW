...
<body>
  <div id="superiore">  
    <div>
    <?php if($username_utente & ($tipologia_abbonamento>0)){?>
  ...
  <div id="inferiore">
    <fieldset id="commenti">
      <?php 
        echo $row['trama'];
        if($username_utente & ($tipologia_abbonamento>0)){
      ?>
      ...
      <?php
          if($username_utente & ($tipologia_abbonamento>0)){
            if(!empty($_POST['aggiungi_commento'])){
              if(!insert_commento_serie($db,"01",$_POST['commento'],"www")){
...
<?php function stampa_commenti_film($db,$codice_film){
  ...
  function insert_commento_serie($db,$codice_serie,$commento,$utente){
    ...
    $ret=pg_execute($db,"insert_commento_serie",array($codice_film,$utente,$commento));
    if(!$ret){
      echo "ERRORE QUERY" . pg_last_error($db);
    }
  }
